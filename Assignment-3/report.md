...
CSV outputs and plots were generated by the benchmark scripts and are included here:

- Sorting timings CSV: [results_sorting.csv](Assignment-3/results_sorting.csv)
- Sorting plots: [sorting_random.png](Assignment-3/sorting_random.png), [sorting_sorted.png](Assignment-3/sorting_sorted.png), [sorting_reversed.png](Assignment-3/sorting_reversed.png), [sorting_repeated.png](Assignment-3/sorting_repeated.png), [sorting_combined.png](Assignment-3/sorting_combined.png)
- Hashing timings CSV: [results_hashing.csv](Assignment-3/results_hashing.csv)
- Hashing plot: [hashing_ops.png](Assignment-3/hashing_ops.png)

Brief observations from the generated data:

- Randomized quicksort consistently shows near $O(n\log n)$ behavior across distributions.
- Deterministic quicksort (first-element pivot) degrades dramatically on already-sorted and reverse-sorted inputs (observed large increases in running time), matching the theoretical $O(n^2)$ worst case.
- Arrays with many repeated elements showed smaller differences between strategies.
- Hash table operations (`insert`, `search`, `delete`) remained small and grew slowly with `n`; dynamic resizing kept load factors low so average operation time stayed near $O(1)$.
